...
class DepartureSearchViewController: UIViewController {
...
extension DepartureSearchViewController : UITableViewDelegate , UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row < stations.count {
            let station: NOAStation = stations[indexPath.row]
            Parameters.departurePoint = station
            DispatchQueue.main.async {
                self.performSegue(withIdentifier: "jumpToArrivalSearch", sender: nil)
            }
        }
    }
